d3.sankey = function() {
      ...
      function forwardLink(part, d) {
        var x0 = d.flag ?
              (Math.sqrt(leftLength) % 1 === 0 ?
                d.source.x + d.source.dx : (leftLength <= 3 ? d.source.x + d.source.dx + 85 / 425 * leftLength * 300 - 60 : d.source.x + d.source.dx + 240)) :
                d.source.x + d.source.dx,
            x1 = d.target.x + 3,
    ...
    function computeNodeValues() {
      let rightLength = nodes.filter((item) => item.layer === 2).length;
      nodes.forEach(function(node) {
        // 设置节点value来影响link的厚度
        if (!(node.value)) {
          node.value = node.flag ? 6.8 : 4.7;
        }
        // node.value = Math.max( 
        ...
        nodesByBreadth.forEach(function(nodes, breadth) {
          nodes.forEach(function(node, index) {
            if (node.targetLinks.length) {
              ...
              let remainPlace = size[1] - nodes[nodes.length - 1].y - nodes[nodes.length - 1].dy - 32;
              if (remainPlace > 0) {
                node.y += (remainPlace / (nodes.length + 1)) * (index + 1);
              }
            }
          });
        });
        ...
        nodesByBreadth.slice().reverse().forEach(function(nodes) {
          nodes.forEach(function(node, index) {
            if (node.sourceLinks.length) {
              ...
              let remainPlace = size[1] - nodes[nodes.length - 1].y - nodes[nodes.length - 1].dy - 12;
              if (index !== 0 && remainPlace > 0) {
                node.y += (remainPlace / (nodes.length - 1)) * index;
              } else {
                node.y += 1.5 * (index + 1);
              }
            }
    ...
    function computeLinkDepths() {
      ...
      let leftLength = nodes.filter((item) => item.layer !== 2).length;
      nodes.forEach(function(node) {
        var sy = 0, ty = 0;
        let slength = node.sourceLinks.length;
        let tlength = node.targetLinks.length;
        // 设置初始位置使得link条居中
        node.sourceLinks.forEach(function(link) {
          link.sy = node.flag ? sy + (85 - 12.8 * slength) / 2 : sy + (55 - 12.8 * slength) / 2;
          sy += link.dy;
        });
        node.targetLinks.forEach(function(link) {
          link.ty = ty + (55 - 12.8 * tlength) / 2;
          ty += link.dy;
        });
      });
